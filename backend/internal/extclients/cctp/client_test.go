...
func Test_Fees(t *testing.T) {
	...
	client := cctp.NewClient()
	ethereumDomain := "0"
	baseDomain := "6"

	fees, err := client.Fees(context.Background(), ethereumDomain, baseDomain)
	r.NoError(err)
...
func Test_MessageAndAttestation(t *testing.T) {
	...
	message, attestation, err := client.MessageAndAttestation(context.Background(), srcDomain, txHash)
	r.NoError(err)
