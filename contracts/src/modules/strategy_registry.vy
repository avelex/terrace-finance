...
def _execute_strategy(
    ...
    balance_before: int256 = convert(
        staticcall IERC20(ledger.USDC).balanceOf(self), int256
    )
    raw_call(
        _target,
...
def is_strategy_allowed(
    target: address,
    selector: Bytes[4],
    proof: DynArray[bytes32, merkle_proof_verification._DYNARRAY_BOUND],
) -> bool:
...
@external
@pure
def encode_supply_aave_v3(
    asset: address, amount: uint256, onBehalfOf: address, referrer: uint16
) -> Bytes[256]:
